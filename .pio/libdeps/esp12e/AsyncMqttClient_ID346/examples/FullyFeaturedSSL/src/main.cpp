...
void onMqttConnect(bool sessionPresent) {
  ...
  Serial.DEBUGLOG("Session present: ");
  Serial.println(sessionPresent);
  uint16_t packetIdSub = mqttClient.subscribe("test/lol", 2);
  Serial.DEBUGLOG("Subscribing at QoS 2, packetId: ");
  Serial.println(packetIdSub);
  mqttClient.publish("test/lol", 0, true, "test 1");
  Serial.println("Publishing at QoS 0");
  uint16_t packetIdPub1 = mqttClient.publish("test/lol", 1, true, "test 2");
  Serial.DEBUGLOG("Publishing at QoS 1, packetId: ");
  Serial.println(packetIdPub1);
  uint16_t packetIdPub2 = mqttClient.publish("test/lol", 2, true, "test 3");
  Serial.DEBUGLOG("Publishing at QoS 2, packetId: ");
...
void onMqttMessage(char* topic, char* payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index, size_t total) {
  ...
  Serial.DEBUGLOG("  topic: ");
  Serial.println(topic);
  Serial.DEBUGLOG("  qos: ");
  Serial.println(properties.qos);
  Serial.DEBUGLOG("  dup: ");
  Serial.println(properties.dup);
  Serial.DEBUGLOG("  retain: ");
  Serial.println(properties.retain);
  Serial.DEBUGLOG("  len: ");
  Serial.println(len);
  Serial.DEBUGLOG("  index: ");
  Serial.println(index);
  Serial.DEBUGLOG("  total: ");
...
void setup() {
  ...
  if (MQTT_SECURE) {
    mqttClient.addServerFingerDEBUGLOG((const uint8_t[])MQTT_SERVER_FINGERPRINT);
  }
#endif
